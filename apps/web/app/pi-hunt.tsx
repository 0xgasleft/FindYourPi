...
function hunt(engine: Engine, input: string, selectedPosition?: number): HuntResult {
  const mode = modeFor(input); const conversion = convert(input, mode, mode === "hash" ? { hashKeepDigits: 6 } : undefined);
  ...
  const base = { normalized_input: conversion.sequence, pi_dataset_version: engine.manifest.version, digits_indexed: engine.manifest.digitCount };
  ...
  function runSearch(input: string) {
    ...
    window.setTimeout(() => { void (async () => {
      try {
        ...
        const selectedResult = hunt(engine, localResult.normalized_input, availablePositions[0]);
...
function ResultCard({ result, connected, checkingClaim, onClaim, onSelect, onAgain }: { result: HuntResult; connected: boolean; checkingClaim: boolean; onClaim: () => void; onSelect: (position: number) => void; onAgain: () => void }) { if (!result.found) return <StatusCard title="Not in this public index yet." body={`We checked all ${result.digits_indexed.toLocaleString()} committed Pi digits locally. Try a six-digit example or a different word.`} action="Search again" onAction={onAgain} />; if (result.available_occurrence_count === 0) return <StatusCard title="No unclaimed occurrence is available." body={`All ${result.occurrence_count?.toLocaleString()} occurrences of ${result.normalized_input} are already minted on Arc.`} action="Search another sequence" onAction={onAgain} />; return <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="relative mt-6 border border-pi-gold/30 bg-void-800/70 px-6 py-7 text-center backdrop-blur"><CornerBrackets /><div className="font-display text-sm font-bold uppercase tracking-[0.2em] text-pi-gold">Occurrence independently provable</div><div className="mt-3 font-mono text-4xl font-bold text-white">{result.normalized_input}</div><div className="mt-4 flex flex-wrap items-center justify-center gap-4 text-sm text-neutral-400"><span>Position <span className="font-mono text-white">{result.position!.toLocaleString()}</span></span><span>{result.match_length} digits</span>{result.rarity_tier && <RarityBadge tier={result.rarity_tier} />}</div><OccurrencePicker result={result} onSelect={onSelect} /><div className="mt-6 grid gap-px border border-white/10 bg-white/10 text-left sm:grid-cols-3"><ProofFact title="01 / Search" value={`${result.available_occurrence_count?.toLocaleString()} unclaimed / ${result.occurrence_count?.toLocaleString()} total`} /><ProofFact title="02 / Commit" value={`Pi dataset v${result.pi_dataset_version}`} /><ProofFact title="03 / Verify" value="Merkle proof ready" accent /></div><p className="mt-4 break-all font-mono text-[10px] leading-relaxed text-neutral-500">discovery {compact(result.discovery_id!)}</p><p className="mt-2 text-xs text-neutral-500">{result.theoretical_rarity} / {result.achievability?.label} / proof stays in your browser until you submit it.</p><div className="mt-6 flex flex-col items-center gap-3 sm:flex-row sm:justify-center">{connected ? <button onClick={onClaim} disabled={checkingClaim} className="border border-pi-gold bg-pi-gold px-6 py-3 text-sm font-bold uppercase tracking-wide text-void-950 transition hover:bg-pi-amber disabled:cursor-wait disabled:opacity-60">{checkingClaim ? "Checking Arc..." : "Claim on Arc"}</button> : <WalletButton />}<button onClick={onAgain} className="text-sm text-neutral-400 underline underline-offset-4">Search again</button></div>{!connected && <p className="mt-3 text-xs text-neutral-500">Connect your wallet to unlock the Arc claim. Gas is paid in USDC.</p>}</motion.div>; }
