...
contract PiHunterNFT is ERC721, ERC721Enumerable, Ownable, Pausable {
    ...
    mapping(uint256 => string) private _tokenInputs;
    mapping(uint256 => InputKind) private _tokenInputKinds;
    ...
    function tokenInput(uint256 tokenId) external view returns (string memory) {
        _requireOwned(tokenId);
        return _tokenInputs[tokenId];
    }

    function tokenInputKind(uint256 tokenId) external view returns (InputKind) {
        _requireOwned(tokenId);
        return _tokenInputKinds[tokenId];
    }
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        string memory inputKind = _tokenInputKinds[tokenId] == InputKind.Number ? "Number" : "Text";
        string memory submittedInputBase64 = Base64.encode(bytes(_tokenInputs[tokenId]));
        ...
        string memory json = string.concat(
            ...
            '","submitted_input_base64":"', submittedInputBase64,
            '","attributes":[{"trait_type":"Input Type","value":"', inputKind,
    ...
    function claim(
        ...
        string calldata submittedInput,
        InputKind inputKind
    ) external whenNotPaused returns (uint256 tokenId) {
        _requireDisplayInput(submittedInput);
        ...
        tokenId = _finalizeClaim(discoveryId, position, sequence, submittedInput, inputKind, msg.sender);
    }
    ...
    function _requireDisplayInput(string calldata submittedInput) private pure {
        uint256 length = bytes(submittedInput).length;
        require(length != 0, "PiHunterNFT: empty input");
        require(length <= 96, "PiHunterNFT: input too long");
    }
    ...
    function _finalizeClaim(
        ...
        _tokenInputs[tokenId] = submittedInput;
        _tokenInputKinds[tokenId] = inputKind;
